...
- **Mock:** 3 anuncios (`titulo`, `texto`, `imagenFondo`, `textoCta`), cada uno con su imagen de fondo en `banners/` (`SemanaCineClasico.webp`, `EstrenosCienciaFiccion.webp`, `MaratonDeTerror.webp`) y su propio texto de botón (ej. "Ver Cartelera", "Reservar Preventa", "Comprar Entradas"). Se consumen desde `main.js` como un carrusel (ver más abajo), no como una lista fija.
...
- **Flujo relevante:**
  ...
  - **Carrusel de anuncios:** `renderizarAnuncios(datos)` no pinta una lista fija — guarda `datos` en la variable de módulo `anuncios`, arma los puntos indicadores (`#anuncios-indicadores`) y pinta el primer slide (`pintarSlideAnuncioActual()`), que setea `anunciosSlide.style.backgroundImage` (degradado + `imagenFondo` del anuncio) y el HTML del título/texto/CTA. `irAAnuncio(indice)` navega circularmente (`(indice + anuncios.length) % anuncios.length`) y reinicia el autoplay; `reiniciarAutoplayAnuncios()` limpia el `setInterval` anterior y arma uno nuevo cada `INTERVALO_AUTOPLAY_MS` (6000ms) — solo si hay más de un anuncio. Las flechas (`#anuncios-anterior`/`#anuncios-siguiente`) y el click delegado en `#anuncios-indicadores` llaman a `irAAnuncio()`. Si solo hay un anuncio, se agrega la clase `anuncios--unico` para ocultar flechas/indicadores por CSS (no tiene sentido navegar un carrusel de un solo slide).
